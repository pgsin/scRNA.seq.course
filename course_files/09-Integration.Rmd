...
pbmc_integrated <- IntegrateData(anchorset = pbmc_anchors, dims = 1:30)
```
...
```{r int16}
pbmc_integrated <- NormalizeData(pbmc_integrated, verbose = F)
pbmc_integrated <- FindVariableFeatures(pbmc_integrated, selection.method = "vst", nfeatures = 2000, verbose = F)
pbmc_integrated <- ScaleData(pbmc_integrated, verbose = F)
pbmc_integrated <- RunPCA(pbmc_integrated, npcs = 30, verbose = F)
pbmc_integrated <- RunUMAP(pbmc_integrated, reduction = "pca", dims = 1:30, verbose = F)
```
...
DimPlot(pbmc_integrated,reduction = "umap") + plot_annotation(title = "10k 3' PBMC and 10k 5' PBMC cells, before integration")
```
...
DimPlot(pbmc_integrated, reduction = "umap") + plot_annotation(title = "10k 3' PBMC and 10k 5' PBMC cells, after integration (Seurat 3)")
```
...
DimPlot(pbmc_integrated, reduction = "umap", split.by = "orig.ident") + NoLegend()
```
...
pbmc_integrated <- FindNeighbors(pbmc_integrated, dims = 1:30, k.param = 10, verbose = F)
pbmc_integrated <- FindClusters(pbmc_integrated, verbose = F)
DimPlot(pbmc_integrated,label = T) + NoLegend()
```

We can now calculate the number of cells in each cluster that came for either 3' or the 5' dataset: 

```{r int22}
count_table <- table(pbmc_integrated@meta.data$seurat_clusters, pbmc_integrated@meta.data$orig.ident)
count_table
```
...
plot_integrated_clusters(pbmc_integrated) 
```
